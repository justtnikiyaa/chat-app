    ...
    <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${selectedUser ? "max-md:hidden": ''}`}>
      <div className='pb-5'>
        <div className='flex justify-between items-center'>
          ...
          <div className="relative py-2">
            ...
            {showMenu && (
              <div onClick={(e) => e.stopPropagation()} className='absolute top-full right-0 z-20 w-32 p-5 rounded-md bg-[#282142] border border-gray-600 text-gray-100'>
      ...
      <div className='flex flex-col'>
        {filteredUsers.map((user, index) =>{
          const displayName = user.fullname || user.fullName || user.email || 'Unknown';
          return (
          ...
          key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id == user._id && 'bg-[#282142]/50'}`}>
           ...
           {Number(unseenMessages[user._id]) > 0 &&  <p className='absolute top-4 right-4 text-xs h-5 w-5 flex justify-center items-center rounded-full bg-violet-500/50'>{unseenMessages[user._id]}</p>}
          </div>
          )
        })}
